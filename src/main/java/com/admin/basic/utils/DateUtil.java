...
public class DateUtil {
	...
	/**
	 * 获取当前系统时间,format为null时，使用默认时间格式yyyy-MM-dd HH:mm:ss
	 * @param format 时间格式
	 * @return str_date
	 */
	...
	public static Timestamp toTimestamp(String time,Boolean endTime){
		if(time.indexOf(".") != -1){
			if(endTime != null && endTime){
				time = time.replaceAll("\\.", "-")+" 23:59:59.999";
			}
			else{
				time = time.replaceAll("\\.", "-")+" 00:00:00";
			}
		}
		Date date = parseToDate(time, "yyyy-MM-dd HH:mm:ss");
...
/*		Date currentTime = new Date();
		System.out.println(getCurTime(null));
		System.out.println(addHour(currentTime,5));
		System.out.println(currentTime.before(addHour(currentTime,5)));*/
		//String str = "2014.09.01";
		//System.out.println(DateUtil.formatDate(DateUtil.getFirstDayOfMonth(new Date()),"yyyy-MM-dd HH:mm:ss.SSS"));
