...
(function($){
    ...
    BootstrapTable.prototype.initEdit = function(){
    	...
    	this.$body.find('> tr').unbind('click').on('click',function(e){
    		var $tr = $(this);
    		var _this = this;
			if(that.isPass == false || $tr.hasClass('editable-select') || data.length < 1 || $tr.hasClass('no-records-found')){
    			return;
    		}
    		$tr.removeClass('no-records-found');
    		recover(that);
    		
    		//20160602 lizx 如果点击的单元格不可编辑，则直接返回
    		var tdIndex = $tr.children().index($(e.target));//当前点击的td下标
    		...
    		that.$body.find('> tr').not(_this).removeClass('editable-select');
    ...
    function initTrClick(that,_this){
		...
		var value = (rowData[tdOpt.field] == null || (rowData[tdOpt.field]==0?'0':rowData[tdOpt.field]) == '')?'':rowData[tdOpt.field];
...
$(function(){
	...
	bootstrap_select.src = _ctx+"/script/plugin/bootstrapV3/bootstrap-select.js";
